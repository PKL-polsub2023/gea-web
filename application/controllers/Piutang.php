...
class Piutang extends CI_Controller {
	...
	{    
      ...
      public function selectInvoice()
      {
            ...
            if (!empty($datamaster)) {
                  $totalPressure = 0;
                  $count = count($datamaster);
                  foreach ($datamaster as $data) {
                      $totalPressure += $data['preasure'];
                  }
            ...
            if ($count > 0) {
                  $meterAwal = $datamaster[0]['meterawal'];
                  $meterAkhir = $datamaster[$count - 1]['meterakhir'];

                  $vt = $meterAkhir - $meterAwal;
                  $vt_formatted = number_format(floatval($vt), 4, '.', '');

                  $pressure = $averagePressure;         
                  $ap = (1.01325 + $pressure) / 1.01325;
                  $k = (1+(0.002*$pressure));
                  $t = 30;
                  $total = ($vt_formatted * ($ap)*(300.15 / (273.15 + $t)) * $k);
                  $total_formatted = number_format(floatval($total), 3, '.', '');

                  $get_harga_jual = $this->Master_Customer_Model->ubah($mastercustomer_id);
                  $harga_jual = $get_harga_jual['harga_jual'];

                  $total_tagihan = floatval($total_formatted) * $harga_jual;
                  ...
                  $data = [
                        ...
                        'invoice' => $this->Invoice_Model->riwayatInvoice($mastercustomer_id),
      ...
      $total_tagihan_post = $this->input->post('total_tagihan');
      $total_tagihan = str_replace('.', '', $total_tagihan_post);
      $total_terbilang = ucwords(terbilang($total_tagihan));
      $total_rupiah = "Rp " . number_format($total_tagihan, 0, ',', '.');
      $mastercustomer_id = $this->input->post('mastercustomer_id');
      ...
      $fromdate_id = date('d/m/Y', strtotime($fromdate));
      $todate_id = date('d/m/Y', strtotime($todate));

      $getData = $this->Master_Customer_Model->ubah($mastercustomer_id);
      $data = array(
            'total_all' => $total_tagihan,
            'u' => $getData,
            'volume' => $volume,
            'total' => $total_rupiah,
            'terbilang' => $total_terbilang,
      ...
      $this->createInvoice($data, $mastercustomer_id);
      ...
      $html = $this->load->view('admin/piutang/invoice-new/invoice', $data, true);
      $mpdf->WriteHTML($html);
      $mpdf->Output('Piutang-'.$data['no_invoice'].'.pdf', 'I');



      // $this->load->view('admin/piutang/invoice-new/invoice', $data);
      }

      public function createInvoice($data, $mastercustomer_id)
      {
            $data_insert=array(
                  'no_invoice' => $data['no_invoice'],
                  'mastercustomer_id' => $mastercustomer_id,
                  'tgl_dari' => $data['fromdate'],
                  'tgl_ke' => $data['todate'],
                  'dd' => $data['dd'],
            ...
            $this->db->insert('dt_invoice', $data_insert); 
      }

      public function ubahPressure()
      {
            $pressure = $this->input->post('p');
            $fromdate =  $this->input->post('fromdate');
            $todate =  $this->input->post('todate');
            $mastercustomer_id =  $this->input->post('mastercustomer_id');
            ...
            if ($count > 0) {
                  ...
                  $total_tagihan = floatval($total_formatted) * $harga_jual;
                  $total_tagihan_formatted = number_format($total_tagihan, 0, '.', '');
                  ...
                  $data = [
                        ...
                        'pressure' => $pressure,
                        'total_formatted' => $total_formatted,
                        'total_tagihan' => $total_tagihan_formatted,
                        'vt' => $vt_formatted,
                        'ap' => $ap,
                        'k' => $k,
                        't' => $t,
                  ...
                  header('Content-Type: application/json'); // Set header content-type sebagai JSON
